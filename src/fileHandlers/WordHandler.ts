...
export class WordHandler {
  ...
  async import(file: File): Promise<EditorData> {
    try {
      ...
      if (commentXml) {
        editorData.comments = this.parseWordXmlToComments(
          commentXml,
          documentXml
        );
      }
   ...
   */
  private extractCommentRangeText(
    documentXml: string,
    commentRangeTextMap: Map<string, string>
  ): void {
    const parser = new DOMParser();
    const doc = parser.parseFromString(documentXml, "text/xml");

    // 查找所有批注范围标记
    const commentRangeStarts = doc.querySelectorAll(
      "w\\:commentRangeStart, commentRangeStart"
    );
    const commentRangeEnds = doc.querySelectorAll(
      "w\\:commentRangeEnd, commentRangeEnd"
    );

    Console.debug(
      `找到 ${commentRangeStarts.length} 个批注范围开始标记，${commentRangeEnds.length} 个结束标记`
    );
    ...
    commentRangeStarts.forEach((start) => {
      ...
      // 查找对应的结束标记
      const endMarker = Array.from(commentRangeEnds).find(
        (end) =>
   ...
   * @returns 提取的文本内容
   */
  private extractTextBetweenNodes(
    startNode: Element,
    endNode: Element
  ): string {
    let text = "";
   ...
   */
  private parseWordXmlToComments(
    xmlContent: string,
    documentXml?: string
  ): EditorData["comments"] {
    const parser = new DOMParser();
    ...
    commentElements.forEach((comment) => {
      const commentId =
        comment.getAttribute("w:id") || comment.getAttribute("id");
      const commentAuthor =
        comment.getAttribute("w:author") || comment.getAttribute("author");
      const commentDate =
        comment.getAttribute("w:date") || comment.getAttribute("date");
      const commentInitials =
        comment.getAttribute("w:initials") || comment.getAttribute("initials");
      ...
      const commentObj: Comment = {
        ...
        initials:
          commentInitials ||
          (commentAuthor ? commentAuthor.substring(0, 2).toUpperCase() : "UN"),
        resolved: false,
      };
      ...
      if (replyElements.length > 1) {
        ...
        for (let i = 1; i < replyElements.length; i++) {
          ...
          const replyId =
            reply.getAttribute("w:id") || reply.getAttribute("id");
          const replyAuthor =
            reply.getAttribute("w:author") || reply.getAttribute("author");
          const replyDate =
            reply.getAttribute("w:date") || reply.getAttribute("date");
          ...
          commentObj.replies.push({
            id:
              replyId ||
              `reply-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
   ...
   * @returns Editor.js数据
   */
  private parseWordXmlToEditorData(
    xmlContent: string,
    commentsXml?: string
  ): EditorData {
    const parser = new DOMParser();
    ...
    if (commentsXml) {
      const parsedComments = this.parseWordXmlToComments(
        commentsXml,
        xmlContent
      );
  ...
  private processParagraphElement(
    ...
    const styleVal =
      styleElement?.getAttribute("w:val") || styleElement?.getAttribute("val");
    ...
    const commentRefs = paragraphElement.querySelectorAll(
    ...
    if (hasFormattedContent) {
      ...
      //   .map(run => run.text || '')
      //   .join('');
      combinedText = this.convertRunsToEditorFormat(formattedRuns);
    } else {
    ...
    const hasSpecialContent = formattedRuns.some(
      (run) =>
        run.type === "bookmark" ||
        run.type === "comment" ||
        run.type === "media"
    ...
    if (paragraphStyles && Object.keys(paragraphStyles).length > 0) {
      blockData.data.styles = paragraphStyles;
    }
    ...
    if (styleVal && styleVal.startsWith("Heading")) {
      ...
      styleVal &&
      (styleVal.includes("List") || styleVal.includes("Bullet"))
    ) {
      // 处理列表样式
      blockData.type = "list";
      blockData.data.style = styleVal.includes("Number")
        ? "ordered"
        : "unordered";
  ...
  private parseTableProperties(tblPrElement: Element | null): any {
    ...
    // 表格样式引用
    const tblStyleElement = tblPrElement.querySelector(
      "w\\:tblStyle, tblStyle"
    );
    if (tblStyleElement) {
      styles.styleReference =
        tblStyleElement.getAttribute("w:val") ||
        tblStyleElement.getAttribute("val");
    }
    ...
    if (tblWElement) {
      const width =
        tblWElement.getAttribute("w:w") || tblWElement.getAttribute("w");
      const type =
        tblWElement.getAttribute("w:type") || tblWElement.getAttribute("type");
      if (width && type) {
        styles.width = {
    ...
    if (jcElement) {
      styles.alignment =
        jcElement.getAttribute("w:val") || jcElement.getAttribute("val");
    }

    // 表格边框
    const tblBordersElement = tblPrElement.querySelector(
  ...
  private parseTableBorders(bordersElement: Element): any {
    ...
    borderTypes.forEach((borderType) => {
      const borderElement = bordersElement.querySelector(
        `w\\:${borderType}, ${borderType}`
      );
      if (borderElement) {
        borders[borderType] = {
          style:
            borderElement.getAttribute("w:val") ||
            borderElement.getAttribute("val"),
          size:
            borderElement.getAttribute("w:sz") ||
            borderElement.getAttribute("sz"),
          color:
            borderElement.getAttribute("w:color") ||
            borderElement.getAttribute("color"),
        };
      }
    });
    ...
    const blocksToHtml = (blocks: any[]) => {
      return blocks
        .map((block) => {
          ...
          if (block.data && block.data.htmlContent) {
            ...
            switch (block.type) {
              ...
              case "list":
                ...
                return htmlContent.startsWith("<div>")
                  ? htmlContent
                  : `<div>${htmlContent}</div>`;
              case "quote":
                if (
          ...
          if (
            block.data &&
            block.data.formattedRuns &&
            block.data.formattedRuns.length > 0
          ) {
            // 使用格式化运行构建HTML
              ...
              .map((run: any) => {
                ...
                if (run.styles) {
                  ...
                  if (runStyles.length > 0) {
                    runHtml = `<span style="${runStyles.join(
                      "; "
                    )}">${runHtml}</span>`;
                  }
                }
          ...
          switch (block.type) {
            case "header":
              const level = block.data.level || 1;
              const headerStyle = `font-weight:bold; margin:12px 0;${
                blockStyles ? ` ${blockStyles}` : ""
              }`;
              return `<h${level} style="${headerStyle}">${processedText}</h${level}>`;
            case "paragraph":
              const paragraphStyle = `line-height:1.6; margin:8px 0;${
                blockStyles ? ` ${blockStyles}` : ""
              }`;
              return `<p style="${paragraphStyle}">${processedText}</p>`;
            case "list":
              const tag = block.data.style === "ordered" ? "ol" : "ul";
              const listStyle = `padding-left:20px;${
                blockStyles ? ` ${blockStyles}` : ""
              }`;
            ...
            case "image":
              const figureStyle = `text-align:center;${
                blockStyles ? ` ${blockStyles}` : ""
              }`;
            ...
            case "quote":
              const quoteStyle = `border-left:4px solid #ccc; padding-left:16px; margin:16px 0; font-style:italic;${
                blockStyles ? ` ${blockStyles}` : ""
              }`;
              return `<blockquote style="${quoteStyle}">${this.processInlineFormatting(
                block.data.text || ""
              )}</blockquote>`;
            case "code":
              const codeStyle = `background-color:#f5f5f5; padding:12px; border-radius:4px; font-family:monospace;${
                blockStyles ? ` ${blockStyles}` : ""
              }`;
              return `<pre style="${codeStyle}"><code>${this.escapeHtml(
                block.data.code || ""
              )}</code></pre>`;
            case "table":
                ...
                const tableRows = block.data.content
                  .map((row: string[]) => {
                    const cells = row
                      .map(
                        (cell) =>
                          `<td style="border:1px solid #ddd; padding:8px;">${this.processInlineFormatting(
                            cell
                          )}</td>`
                      )
                      .join("");
            ...
            default:
              ...
              if (block.data && block.data.text) {
                const defaultStyle = `margin:8px 0;${
                  blockStyles ? ` ${blockStyles}` : ""
                }`;
                return `<div style="${defaultStyle}">${this.processInlineFormatting(
                  block.data.text
                )}</div>`;
              }
              return "";
  ...
  private htmlToWordXml(htmlString: string): string {
    ...
    const processNode = (node: Node): string => {
      ...
      const element = node as Element;
      const tagName = element.tagName.toLowerCase();
      const styles = this.parseInlineStyles(
          ...
          return `<w:p>
            ...
            <w:r>
              <w:rPr>
                ...
                ${
                  styles.color
                    ? `<w:color w:val="${this.convertColor(styles.color)}"/>`
                    : ""
                }
              </w:rPr>
          ...
          Array.from(element.children).forEach((li) => {
            if (li.tagName.toLowerCase() === "li") {
              listContent += `<w:p>
                ...
                <w:r>
                  <w:t xml:space="preserve">${this.escapeXml(
                    li.textContent || ""
                  )}</w:t>
                </w:r>
              </w:p>`;
            ...
            if (row.tagName.toLowerCase() === "tr") {
              ...
              Array.from(row.children).forEach((cell) => {
                if (cell.tagName.toLowerCase() === "td") {
                  tableContent += `<w:tc><w:tcPr><w:tcW w:w="0" w:type="auto"/></w:tcPr><w:p><w:r><w:t>${this.escapeXml(
                    cell.textContent || ""
                  )}</w:t></w:r></w:p></w:tc>`;
                }
              });
    ...
    Array.from(node.childNodes).forEach((child) => {
      if (child.nodeType === Node.TEXT_NODE) {
        ...
        const childStyles = Object.assign(
          {},
          parentStyles,
          this.parseInlineStyles(element.getAttribute("style") || "")
        );
  ...
  private createRun(text: string, styles: Record<string, string>): string {
    ...
    if (backgroundColor) {
      ...
      if (bgColorLower === "ffff00" || bgColorLower === "yellow") {
        ...
        bgColorLower === "ff00ff" ||
        bgColorLower === "magenta" ||
        bgColorLower === "fuchsia"
      ) {
        ...
        bgColorLower === "f5f5f5" ||
        bgColorLower === "lightgray" ||
        bgColorLower === "lightgrey"
      ) {
    ...
    return `<w:r>
      <w:rPr>
        ...
        ${
          styles["font-family"]
            ? `<w:rFonts w:ascii="${styles["font-family"]}" w:hAnsi="${styles["font-family"]}"/>`
            : ""
        }
      </w:rPr>
...
<w:fonts xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
  ...
  private parseParagraphProperties(pPrElement: Element | null): any {
    ...
    if (spacingElement) {
      const line =
        ...
        spacingElement.getAttribute("line");
      const lineRule =
        spacingElement.getAttribute("w:lineRule") ||
        spacingElement.getAttribute("lineRule");
      ...
      const spacingBefore =
        ...
        spacingElement.getAttribute("before");
      const spacingAfter =
        spacingElement.getAttribute("w:after") ||
        spacingElement.getAttribute("after");
    ...
    if (indElement) {
      const left =
        indElement.getAttribute("w:left") || indElement.getAttribute("left");
      const right =
        indElement.getAttribute("w:right") || indElement.getAttribute("right");
      const firstLine =
        indElement.getAttribute("w:firstLine") ||
        indElement.getAttribute("firstLine");
      const hanging =
        indElement.getAttribute("w:hanging") ||
        indElement.getAttribute("hanging");
      const leftChars =
        indElement.getAttribute("w:leftChars") ||
        indElement.getAttribute("leftChars");
      const rightChars =
        indElement.getAttribute("w:rightChars") ||
        indElement.getAttribute("rightChars");
    ...
    if (tabsElement) {
      ...
      tabElements.forEach((tab) => {
        ...
        const leader =
          tab.getAttribute("w:leader") || tab.getAttribute("leader");
    ...
    if (numPrElement) {
      ...
      const level = ilvlElement
        ? parseInt(
            ilvlElement.getAttribute("w:val") ||
              ilvlElement.getAttribute("val") ||
              "0"
          )
        : 0;
      const numId = numIdElement
        ? parseInt(
            numIdElement.getAttribute("w:val") ||
              numIdElement.getAttribute("val") ||
              "0"
          )
        : 0;
    ...
    if (shdElement) {
      const val =
        shdElement.getAttribute("w:val") || shdElement.getAttribute("val");
      const color =
        shdElement.getAttribute("w:color") || shdElement.getAttribute("color");
      const fill =
        shdElement.getAttribute("w:fill") || shdElement.getAttribute("fill");
    ...
    if (pStyleElement) {
      const styleId =
        pStyleElement.getAttribute("w:val") ||
        pStyleElement.getAttribute("val");
      if (styleId) {
    ...
    if (keepNextElement) {
      const val =
        keepNextElement.getAttribute("w:val") ||
        keepNextElement.getAttribute("val");
      styles.keepNext = val !== "0" && val !== "false";
    }

    const keepLinesElement = pPrElement.querySelector(
      "w\\:keepLines, keepLines"
    );
    if (keepLinesElement) {
      const val =
        keepLinesElement.getAttribute("w:val") ||
        keepLinesElement.getAttribute("val");
      styles.keepLines = val !== "0" && val !== "false";
    }

    const pageBreakBeforeElement = pPrElement.querySelector(
      "w\\:pageBreakBefore, pageBreakBefore"
    );
    if (pageBreakBeforeElement) {
      const val =
        pageBreakBeforeElement.getAttribute("w:val") ||
        pageBreakBeforeElement.getAttribute("val");
      styles.pageBreakBefore = val !== "0" && val !== "false";
    }

    const widowControlElement = pPrElement.querySelector(
      "w\\:widowControl, widowControl"
    );
    if (widowControlElement) {
      const val =
        widowControlElement.getAttribute("w:val") ||
        widowControlElement.getAttribute("val");
      styles.widowControl = val !== "0" && val !== "false";
    }

    // 解析文本对齐方式
    const textAlignmentElement = pPrElement.querySelector(
      "w\\:textAlignment, textAlignment"
    );
    if (textAlignmentElement) {
      const val =
        textAlignmentElement.getAttribute("w:val") ||
        textAlignmentElement.getAttribute("val");
      if (val) {
    ...
    if (kinsokuElement) {
      const val =
        kinsokuElement.getAttribute("w:val") ||
        kinsokuElement.getAttribute("val");
    ...
    if (wordWrapElement) {
      const val =
        wordWrapElement.getAttribute("w:val") ||
        wordWrapElement.getAttribute("val");
      styles.wordWrap = val !== "0" && val !== "false";
    }

    const overflowPunctElement = pPrElement.querySelector(
      "w\\:overflowPunct, overflowPunct"
    );
    if (overflowPunctElement) {
      const val =
        overflowPunctElement.getAttribute("w:val") ||
        overflowPunctElement.getAttribute("val");
      styles.overflowPunct = val !== "0" && val !== "false";
    }

    const topLinePunctElement = pPrElement.querySelector(
      "w\\:topLinePunct, topLinePunct"
    );
    if (topLinePunctElement) {
      const val =
        topLinePunctElement.getAttribute("w:val") ||
        topLinePunctElement.getAttribute("val");
      styles.topLinePunct = val !== "0" && val !== "false";
    }

    const autoSpaceDEElement = pPrElement.querySelector(
      "w\\:autoSpaceDE, autoSpaceDE"
    );
    if (autoSpaceDEElement) {
      const val =
        autoSpaceDEElement.getAttribute("w:val") ||
        autoSpaceDEElement.getAttribute("val");
      styles.autoSpaceDE = val !== "0" && val !== "false";
    }

    const autoSpaceDNElement = pPrElement.querySelector(
      "w\\:autoSpaceDN, autoSpaceDN"
    );
    if (autoSpaceDNElement) {
      const val =
        autoSpaceDNElement.getAttribute("w:val") ||
        autoSpaceDNElement.getAttribute("val");
    ...
    if (bidiElement) {
      const val =
        bidiElement.getAttribute("w:val") || bidiElement.getAttribute("val");
      styles.bidi = val !== "0" && val !== "false";
    }

    // 解析右缩进调整
    const adjustRightIndElement = pPrElement.querySelector(
      "w\\:adjustRightInd, adjustRightInd"
    );
    if (adjustRightIndElement) {
      const val =
        adjustRightIndElement.getAttribute("w:val") ||
        adjustRightIndElement.getAttribute("val");
      styles.adjustRightInd = val !== "0" && val !== "false";
    }

    // 解析网格对齐
    const snapToGridElement = pPrElement.querySelector(
      "w\\:snapToGrid, snapToGrid"
    );
    if (snapToGridElement) {
      const val =
        snapToGridElement.getAttribute("w:val") ||
        snapToGridElement.getAttribute("val");
  ...
  private parseRunProperties(rPrElement: Element | null): any {
    ...
    if (szElement) {
      const size =
        szElement.getAttribute("w:val") || szElement.getAttribute("val");
      if (size) {
    ...
    if (szCsElement) {
      const size =
        szCsElement.getAttribute("w:val") || szCsElement.getAttribute("val");
      if (size) {
    ...
    if (colorElement) {
      const color =
        colorElement.getAttribute("w:val") || colorElement.getAttribute("val");
      const themeColor =
        colorElement.getAttribute("w:themeColor") ||
        colorElement.getAttribute("themeColor");
      const themeTint =
        colorElement.getAttribute("w:themeTint") ||
        colorElement.getAttribute("themeTint");
      const themeShade =
        colorElement.getAttribute("w:themeShade") ||
        colorElement.getAttribute("themeShade");
    ...
    // 解析14版本的文本填充效果
    const textFillElement = rPrElement.querySelector(
      "w14\\:textFill, textFill"
    );
    if (textFillElement) {
      const solidFillElement = textFillElement.querySelector(
        "w14\\:solidFill, solidFill"
      );
      if (solidFillElement) {
        const schemeClrElement = solidFillElement.querySelector(
          "w14\\:schemeClr, schemeClr"
        );
        if (schemeClrElement) {
          const val =
            schemeClrElement.getAttribute("w14:val") ||
            schemeClrElement.getAttribute("val");
          if (val) {
            styles.textFill = {
              type: "solid",
              schemeColor: val,
    ...
    if (boldElement) {
      const val =
        boldElement.getAttribute("w:val") || boldElement.getAttribute("val");
      if (val !== "0" && val !== "false") {
    ...
    if (bCsElement) {
      const val =
        bCsElement.getAttribute("w:val") || bCsElement.getAttribute("val");
      if (val !== "0" && val !== "false") {
    ...
    if (italicElement) {
      const val =
        italicElement.getAttribute("w:val") ||
        italicElement.getAttribute("val");
      if (val !== "0" && val !== "false") {
    ...
    if (iCsElement) {
      const val =
        iCsElement.getAttribute("w:val") || iCsElement.getAttribute("val");
      if (val !== "0" && val !== "false") {
    ...
    if (underlineElement) {
      ...
      const color =
        underlineElement.getAttribute("w:color") ||
        underlineElement.getAttribute("color");
    ...
    if (strikeElement) {
      const val =
        strikeElement.getAttribute("w:val") ||
        strikeElement.getAttribute("val");
      if (val !== "0" && val !== "false") {
    ...
    if (dstrikeElement) {
      const val =
        dstrikeElement.getAttribute("w:val") ||
        dstrikeElement.getAttribute("val");
      if (val !== "0" && val !== "false") {
        styles.textDecoration = styles.textDecoration
          ? `${styles.textDecoration} line-through`
          : "line-through";
    ...
    // 解析高亮颜色
    const highlightElement = rPrElement.querySelector(
      "w\\:highlight, highlight"
    );
    if (highlightElement) {
      const val =
        highlightElement.getAttribute("w:val") ||
        highlightElement.getAttribute("val");
      if (val && val !== "none") {
    ...
    // 解析上标/下标
    const vertAlignElement = rPrElement.querySelector(
      "w\\:vertAlign, vertAlign"
    );
    if (vertAlignElement) {
      const val =
        vertAlignElement.getAttribute("w:val") ||
        vertAlignElement.getAttribute("val");
      if (val === "superscript") {
    ...
    if (spacingElement) {
      const val =
        spacingElement.getAttribute("w:val") ||
        spacingElement.getAttribute("val");
      if (val) {
    ...
    if (wElement) {
      const val =
        wElement.getAttribute("w:val") || wElement.getAttribute("val");
      if (val) {
    ...
    if (positionElement) {
      const val =
        positionElement.getAttribute("w:val") ||
        positionElement.getAttribute("val");
      if (val) {
        // 单位是 half-points
    ...
    if (bdrElement) {
      const val =
        bdrElement.getAttribute("w:val") || bdrElement.getAttribute("val");
      const sz =
        bdrElement.getAttribute("w:sz") || bdrElement.getAttribute("sz");
      const color =
        bdrElement.getAttribute("w:color") || bdrElement.getAttribute("color");
    ...
    if (effectElement) {
      const val =
        effectElement.getAttribute("w:val") ||
        effectElement.getAttribute("val");
      if (val) {
    ...
    if (embossElement) {
      const val =
        embossElement.getAttribute("w:val") ||
        embossElement.getAttribute("val");
      if (val !== "0" && val !== "false") {
    ...
    if (imprintElement) {
      const val =
        imprintElement.getAttribute("w:val") ||
        imprintElement.getAttribute("val");
      if (val !== "0" && val !== "false") {
    ...
    if (shadowElement) {
      const val =
        shadowElement.getAttribute("w:val") ||
        shadowElement.getAttribute("val");
      if (val !== "0" && val !== "false") {
    ...
    if (outlineElement) {
      const val =
        outlineElement.getAttribute("w:val") ||
        outlineElement.getAttribute("val");
      if (val !== "0" && val !== "false") {
    ...
    // 解析小型大写字母
    const smallCapsElement = rPrElement.querySelector(
      "w\\:smallCaps, smallCaps"
    );
    if (smallCapsElement) {
      const val =
        smallCapsElement.getAttribute("w:val") ||
        smallCapsElement.getAttribute("val");
      if (val !== "0" && val !== "false") {
    ...
    if (capsElement) {
      const val =
        capsElement.getAttribute("w:val") || capsElement.getAttribute("val");
      if (val !== "0" && val !== "false") {
    ...
    if (vanishElement) {
      const val =
        vanishElement.getAttribute("w:val") ||
        vanishElement.getAttribute("val");
      if (val !== "0" && val !== "false") {
    ...
    if (langElement) {
      const val =
        langElement.getAttribute("w:val") || langElement.getAttribute("val");
      const eastAsia =
        langElement.getAttribute("w:eastAsia") ||
        langElement.getAttribute("eastAsia");
      const bidi =
        langElement.getAttribute("w:bidi") || langElement.getAttribute("bidi");
  ...
  private extractFontDefinitions(rFontsElement: Element): any {
    ...
    const hAnsi =
      ...
      rFontsElement.getAttribute("hAnsi");
    const eastAsia =
      rFontsElement.getAttribute("w:eastAsia") ||
      rFontsElement.getAttribute("eastAsia");
    const cs =
      rFontsElement.getAttribute("w:cs") || rFontsElement.getAttribute("cs");
    const hint =
      rFontsElement.getAttribute("w:hint") ||
      rFontsElement.getAttribute("hint");
    ...
    for (let i = 0; i < childNodes.length; i++) {
      ...
      if (node.nodeType === Node.ELEMENT_NODE) {
        ...
        // 处理书签开始 (w:bookmarkStart)
          ...
          const bookmarkId =
            element.getAttribute("w:id") || element.getAttribute("id");
          const bookmarkName =
    ...
    for (let i = 0; i < childNodes.length; i++) {
      ...
      if (node.nodeType === Node.ELEMENT_NODE) {
        ...
        // 处理换行符 (w:br)
          ...
          const type =
            element.getAttribute("w:type") || element.getAttribute("type");
          const clear =
            element.getAttribute("w:clear") || element.getAttribute("clear");
        ...
        // 处理符号字符 (w:sym)
          ...
          const font =
            element.getAttribute("w:font") || element.getAttribute("font");
          const char =
            element.getAttribute("w:char") || element.getAttribute("char");
        ...
        // 处理尾注引用 (w:endnoteReference)
          ...
          tagName === "w:endnotereference" ||
          tagName === "endnotereference"
        ) {
          const endnoteId =
            element.getAttribute("w:id") || element.getAttribute("id");
          textContent.push({
  ...
  private parseHyperlink(hyperlinkElement: Element): any[] {
    ...
    const anchor =
      ...
      hyperlinkElement.getAttribute("anchor");
    const tooltip =
      hyperlinkElement.getAttribute("w:tooltip") ||
      hyperlinkElement.getAttribute("tooltip");
